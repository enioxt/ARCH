...
export async function getBudget(budgetId: string): Promise<BudgetReport | null> {
  ...
  const budget: BudgetReport = {
    ...
    scenarios: [
      { scenario: 'economico', total: parseFloat(budgetRow.total_low), items: [] },
      { scenario: 'padrao', total: parseFloat(budgetRow.total_mid), items: [] },
      { scenario: 'premium', total: parseFloat(budgetRow.total_high), items: [] },
    ],
    items,
